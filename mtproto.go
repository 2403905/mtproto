...
const (
	appId   = 78941
	appHash = "6decb28f01d47bb3baf51ab24b29f3a1"
	//  defaultServerAddr = "149.154.175.10:80" // Test
...
func NewMTProto(authkeyfile string, ipv6 bool, rpcServerMode bool) (*MTProto, error) {
	var err error
	m := new(MTProto)
	m.ipv6 = ipv6
	m.runningRPCServer = rpcServerMode
...
func (m *MTProto) Auth(phonenumber string) error {
	...
	for flag {
		...
		case TL_rpc_error:
			x := x.(TL_rpc_error)
			if x.error_code != 303 {
				return fmt.Errorf("RPC error_code: %d, %s\n", x.error_code, x.error_message)
			}
			var newDc int32
			...
			if n != 1 {
				...
				if n != 1 {
					return fmt.Errorf("RPC error_string: %s\n", x.error_message)
				}
			}

			newDcAddr, ok := m.dclist[newDc]
			if !ok {
				return fmt.Errorf("Wrong DC index: %d\n", newDc)
			}
...
func (m *MTProto) ChannelParticipants(params []string) ([]TL, error) {
	var err error
	channelId := 0
	offset := 1
	limit := 10
	...
	if channelId, err = strconv.Atoi(params[0]); err != nil {
		return nil, fmt.Errorf("Invalid channel_id value %v\n", params[0])
	}
	if len(params) >= 2 {
		if offset, err = strconv.Atoi(params[1]); err != nil {
			return nil, fmt.Errorf("Invalid offset value %v\n", params[1])
		}
	}
	if len(params) >= 3 {
		if limit, err = strconv.Atoi(params[2]); err != nil {
			return nil, fmt.Errorf("Invalid limit value %v\n", params[2])
		}
	}
	if len(params) >= 4 {
		if params[3] == "admin" {
			filter = TL_channelParticipantsAdmins{}
		} else if params[3] == "kicked" {
			filter = TL_channelParticipantsKicked{}
		}
	}
	if filter == nil {
	...
	resp := make(chan TL, 1)
	m.queueSend <- packetToSend{TL_channels_getParticipants{
		TL_inputChannel{
			int32(channelId),
			m.accessHashes[int32(channelId)],
		},
		filter,
		int32(offset),
		int32(limit),
	...
	case TL_channels_channelParticipants:
		channelParticipants, _ := x.(TL_channels_channelParticipants)

		fmt.Printf(
			"\033[33m\033[1m%10s    %10s\033[0m\n",
			"id", "date",
		)
		for _, v := range channelParticipants.participants {
			switch v.(type) {
			case TL_channelParticipant:
				v := v.(TL_channelParticipant)
				fmt.Printf(
					"%10d    %10d\n",
					v.user_id,
					v.date,
				)
			case TL_channelParticipantCreator:
				v := v.(TL_channelParticipantCreator)
				fmt.Printf(
					"%10d   %10d\n",
					v.user_id,
					0,
		...
		return channelParticipants.participants, nil

	case TL_rpc_error:
		err := x.(TL_rpc_error)
		return nil, fmt.Errorf("Code: %d - %s\n", err.error_code, err.error_message)
	default:
		return nil, fmt.Errorf("RPC: %#v\n", x)
	}
}
...
func (m *MTProto) GetContacts() (TL, error) {
	...
	x := <-resp
	switch x.(type) {
	case TL_contacts_contacts:
		list, _ := x.(TL_contacts_contacts)
		fmt.Printf(
...
func (m *MTProto) Hasshes() error {
	fmt.Printf(
		"\033[33m\033[1m%10s    %10s\033[0m\n",
		"channel_id", "hash",
	)
	for channelId, hash := range m.accessHashes {
		fmt.Printf(
			"%10d	%20d\n",
			channelId, hash,
		)
...
func (m *MTProto) GetHistory(id int32) error{
	resp := make(chan TL, 1)
	peer, _ := m.parsePeerById("419095468")
	m.queueSend <- packetToSend{
	...
	x := <-resp
	return fmt.Errorf("GetDialogs RPC: %#+v\n", x)
}
/*
...
func (m *MTProto) GetMessages(chat_id int32) error{
	resp := make(chan TL, 1)
	m.queueSend <- packetToSend{
	...
	x := <-resp
	list, ok := x.(TL_messages_messages)
...
func (m *MTProto) ReadMessageContents(id int32) error{
	resp := make(chan TL, 1)
	m.queueSend <- packetToSend{
	...
	x := <-resp
	return fmt.Errorf("ReadMessageContents RPC: %#+v\n", x)
}

func (m *MTProto) UpdateState() error{
	resp := make(chan TL, 1)
	m.queueSend <- packetToSend{
	...
	return fmt.Errorf("UpdateState RPC: %#+v\n",x)
}

func (m *MTProto) GetDialogs() error {
	resp := make(chan TL, 1)
	m.queueSend <- packetToSend{TL_messages_getDialogs{0, 0, TL_inputPeerSelf{}, 40}, resp}
	x := <-resp
	...
	//fmt.Printf("GetDialogs RPC: %#v\n", x)

	//users
	var users map[int32]TL
	users = make(map[int32]TL)
	for _, user := range list.users {
		user := user.(TL_user)
		users[user.id] = user
	}
	//chats
	var chats map[int32]TL
	chats = make(map[int32]TL)
	for _, chat := range list.chats {
		switch chat.(type) {
		case TL_chatEmpty:
			chat := chat.(TL_chatEmpty)
			chats[chat.id] = chat
		case TL_chat:
			chat := chat.(TL_chat)
			chats[chat.id] = chat
		case TL_chatForbidden:
			chat := chat.(TL_chatForbidden)
			chats[chat.id] = chat
		//case TL_channelEmpty:
		case TL_channel:
			chat := chat.(TL_channel)
			chats[chat.id] = chat
	...
	for _, message := range list.messages {
		...
		case TL_messageEmpty:
			message := message.(TL_messageEmpty)
			messages[message.id] = "TL_messageEmpty"
		/*
		case TL_messageForwarded:
			message := message.(TL_messageForwarded)
			var to_id int32
			switch message.to_id.(type) {
			case TL_peerUser:
				to_id = message.to_id.(TL_peerUser).user_id
			case TL_peerChat:
				to_id = message.to_id.(TL_peerChat).chat_id
			case TL_peerChannel:
				to_id = message.to_id.(TL_peerChannel).channel_id
			}
			messages[to_id] = "TL_messageForwarded"
		*/
		case TL_message:
			message := message.(TL_message)
			var to_id int32
			switch message.to_id.(type) {
			case TL_peerUser:
				to_id = message.to_id.(TL_peerUser).user_id
			...
			messages[to_id] = "TL_message"
		case TL_messageService:
			message := message.(TL_messageService)
			var to_id int32
			switch message.to_id.(type) {
			case TL_peerUser:
				to_id = message.to_id.(TL_peerUser).user_id
			...
			messages[to_id] = "TL_messageService"
		}
	}

	fmt.Printf(
		"\033[33m\033[1m%10s    \033[33m\033[1m%10s    %10s    %-10s    %-5s	%-20s\033[0m\n",
		"idx", "id", "type", "top_message", "unread_count", "title",
	)
	t := ""
	i := int32(0)
	title := ""
					...
					case TL_peerChat:
						...
						switch chats[i].(type) {
							case TL_chatEmpty:
								title = "Empty"
							case TL_chat:
								title = chats[i].(TL_chat).title
							case TL_chatForbidden:
								title = chats[i].(TL_chatForbidden).title
						}
						chat_idx = chat_idx + 1
						idx = chat_idx
				}
				fmt.Printf(
					"%10d	%10d	%8s	%-10d	%-5d	%-20s\n",
					idx, i, t, v.top_message, v.unread_count, messages[i]+"#"+title,
				)
			case TL_dialogChannel:
				...
				switch chats[i].(type) {
					case TL_chatEmpty:
						title = "Empty"
					case TL_channel:
						ch := chats[i].(TL_channel)
						title = ch.title
						m.accessHashes[ch.id] = ch.access_hash
					case TL_channelForbidden:
						ch := chats[i].(TL_channelForbidden)
						title = ch.title
						m.accessHashes[ch.id] = ch.access_hash
				}
				chat_idx = chat_idx + 1
...
// func (m *MTProto) ContactAddMultiple(phoneNumber string, firstName string, lastName string) error {
//
// }
func (m *MTProto) ContactAdd(args []string) (TL, error) {
	resp := make(chan TL, 1)

	if len(args) < 3 {
		return nil, fmt.Errorf("Usage contact_add <phone> <first_name> <last_name>")
	}
	...
	inputContacts := make([]TL, 1)
	inputContacts[0] = TL_inputPhoneContact{
		rand.Int63(),
		phone,
		firstName,
		lastName,
	...
	x := <-resp
	importedContacts, ok := x.(TL_contacts_importedContacts)
	...
	if !ok {
		...
		for _, v := range importedContacts.imported {
			fmt.Printf(
				"%10d    %10d\n",
				v.user_id,
				v.client_id,
...
func (m *MTProto) ChannelCreate(args []string) (TL, error) {

	if len(args) < 1 {
		return nil, fmt.Errorf("Usage channel_create <name> <about>*")
	}
	title := args[0]
	about := ""
	if len(args) > 1 {
	...
	switch x.(type) {
	case TL_updates:
		tl_updates := x.(TL_updates)
		var tl_channel TL_channel
	Loop:
		for _, chat := range tl_updates.chats {
			switch chat.(type) {
			case TL_channel:
				tl_channel = chat.(TL_channel)
				break Loop
			default:
				break
			}
		}
		return tl_channel, nil
	case TL_rpc_error:
		err := x.(TL_rpc_error)
		return nil, fmt.Errorf("Code: %d - %s", err.error_code, err.error_message)
	default:
		return nil, fmt.Errorf("RPC %#v", x)
	}
}

func (m *MTProto) ChannelInviteTo(params []string) ([]int32, error) {
	var err error
	if len(params) < 2 {
		return nil, fmt.Errorf("Usage channel_invite_to <channel_id> <user_id>+")
	}
	channelId := 0
	// this validation could be better
	if channelId, err = strconv.Atoi(params[0]); err != nil {
		return nil, fmt.Errorf("Invalid channel_id value %v", params[0])
	}

	tl_inputChannel := TL_inputChannel{
		int32(channelId),
		m.accessHashes[int32(channelId)],
	}

	tl_users := make([]TL, len(params[1:]))
	userId := 0
	for i, v := range params[1:] {
		if userId, err = strconv.Atoi(v); err != nil {
			return nil, fmt.Errorf("Invalid userid value %v", v)
		}
		tl_users[i] = TL_inputUser{
			int32(userId),
			rand.Int63(),
		}
	}
	...
	case TL_updates:
		...
		for _, v := range tl_updates.updates {
			switch v.(type) {
			case TL_updateNewChannelMessage:
				// TODO make this better,
				newChannelMessage := v.(TL_updateNewChannelMessage)
				messageService := newChannelMessage.message.(TL_messageService)
				action := messageService.action.(TL_messageActionChatAddUser)
				users = action.users
			}
		}
		return users, nil
	case TL_rpc_error:
		rpcError := x.(TL_rpc_error)
		return nil, fmt.Errorf("Code: %d - %s", rpcError.error_code, rpcError.error_message)
	default:
		return nil, fmt.Errorf("RPC %#v", x)
	}
}
...
func (m *MTProto) SendMsg(params []string) (TL, error) {
	if len(params) < 2 {
		return nil, fmt.Errorf("Usage msg <peer_id> <message>")
	}
	peer, _ := m.parsePeerById(params[0])
	msg := params[1]
	f := uint32(0)
	f |= 1 << 1
	//	f |= 1 << 3
	...
	x := <-resp
	tl_updateShortSentMessage, ok := x.(TL_updateShortSentMessage)
	fmt.Printf("%v, RPC: %#v", ok, x)
	if !ok {
		return nil, fmt.Errorf("RPC: %#v", x)
	}

	return tl_updateShortSentMessage, nil
}
...
func (m *MTProto) Process(msgId int64, seqNo int32, data interface{}) interface{} {
	...
	// fmt.Fprintln(os.Stderr, "Received: ", reflect.TypeOf(data), data)
	switch data.(type) {
